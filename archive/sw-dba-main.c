		...
		if (now > next_broadcast) {
			...
			if (init_run > 0) {
				...
				if (sched_resp > 0) {
					...
					grant_response1 = create_grant_override_response(pool, cur_counter, elems, bw);
				} else {
					grant_response1 = create_default_grant_override_response(pool, cur_counter, bw);
				}
			...
			for (uint16_t i = 0; i < nb_rx; i++) {
				...
				default:
					printf("%x\n", etype);
                                        stats.port_types[port].ip++;
                                        if (port == traf_port) {
						printf("default: traf_port to tibit\n");
                                                rte_eth_tx_buffer(tibit_ports[0], 0, tx_buffer[tibit_ports[0]], bufs[i]);
                                        } else {
						printf("default: tibit to traf_port\n");
                                                rte_eth_tx_buffer(traf_port, 0, tx_buffer[traf_port], bufs[i]);
                                        }
                                        break;
...
main(int argc, char *argv[])
{
	...
	if (nb_ports != 4)
		rte_exit(EXIT_FAILURE, "This app is build to use 4 ports. 2 physical ports to (a) ctrl (b) traffic and one linux bridge via pcap\n");
	...
	for (uint8_t i = 0; i < discovered_ports_idx; i++) {
		...
		if (strncmp(portname, CTRL_PORT, 12) == 0) {
			// to ctrl port 
			printf("INFO: Found controller port: %d, %s\n", port, portname);
			ctrl_port = port;
		} else if (strncmp(portname, TRAF_PORT,12) == 0) {
			// to traffic port
			printf("INFO: Found Traffic port:  %d, %s\n", port, portname);
			traf_port = port;

...
struct rte_mbuf *
create_default_grant_override_response(struct rte_mempool *pool, uint32_t counter)
{
	static const uint16_t priority = 7;
	static uint64_t tsc_khz;
	static int printed;
	...
	*body = (struct cascading_group_msg){0};
	...
	override_elems[0].size = OVERRIDE_BW;
...
struct rte_mbuf *
create_grant_override_response(struct rte_mempool *pool, uint32_t counter, union sched_elem *elems)
{
	static const uint16_t priority = 7;
	static uint64_t tsc_khz;
	static int printed;
	...
	*body = (struct cascading_group_msg){0};
	...
	if (elems[1].val != 0) {
		...
		/* Set payload size to 32786 bytes */
		override_elems[1].size = OVERRIDE_BW;
	...
	if (elems[2].val != 0) {
		...
		/* Set payload size to 32786 bytes */
		override_elems[2].size = OVERRIDE_BW;
	...
	if (elems[3].val != 0) {
		...
		/* Set payload size to 32786 bytes */
		override_elems[3].size = OVERRIDE_BW;

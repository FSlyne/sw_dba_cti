		...
		const char *param, struct rte_tel_data *d)
{
	...
	rte_pcapng_add_interface(capture_hdl, ctrl_port, NULL, "Controller port", NULL);
	rte_pcapng_add_interface(capture_hdl, traf_port, NULL, "Traffic port", NULL);
		...
		struct rte_tel_data *d)
{
	...
	if (strcmp(cmd, INFO_CMD) == 0) {
		...
		rte_tel_data_add_dict_uint(d, "bcast_interval_usec", broadcast_interval_usec);
                ...
                rte_tel_data_add_dict_uint(d, "traf_port_id", traf_port);
                rte_eth_dev_get_name_by_port(traf_port, portname);
                rte_tel_data_add_dict_string(d, "traf_port_name", portname);

                rte_tel_data_add_dict_uint(d, "ctrl_port_id", ctrl_port);
                rte_eth_dev_get_name_by_port(ctrl_port, portname);
                rte_tel_data_add_dict_string(d, "ctrl_port_name", portname);

                rte_tel_data_add_dict_uint(d, "downstream_port_id0", tibit_ports[0]);
                rte_eth_dev_get_name_by_port(tibit_ports[0], portname);
                rte_tel_data_add_dict_string(d, "downstream_port_name0", portname);
		...
		for (uint16_t i = 0; i < 4; i++) {
			char stat_name[32];
			char *port_names[] = {"Upstream_Traf", "Upstream_Ctrl", "XDP_Q0", "XDP_Q1"};
			snprintf(stat_name, sizeof(stat_name), "%s_Traf_pkts", port_names[i]);
			ADD_NAMED_STAT(stat_name, port_types[i].ip);
                        snprintf(stat_name, sizeof(stat_name), "%s_Ctrl_pkts", port_names[i]);
                        ADD_NAMED_STAT(stat_name, port_types[i].ip);

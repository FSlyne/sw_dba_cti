...
create_grant_override_response(struct rte_mempool *pool, uint32_t counter, union sched_elem *elems, uint32_t bw)
{
	...
	*body = (struct cascading_group_msg){0};
	...
	//if (elems[0].val || elems[1].val || elems[2].val || elems[3].val ) 
        //printf("size %d %d %d %d %d %d %d %d\n", elems[0].link_id, elems[0].size, 
        //                              elems[1].link_id, elems[1].size,
        //                              elems[2].link_id, elems[2].size,
        //                              elems[3].link_id, elems[3].size); 
